...
for %%x in (opencv_world310.dll libiomp5md.dll mklml.dll mkldnn.dll Cntk.Math-%version%.dll Cntk.PerformanceProfiler-%version%.dll Cntk.Core-%version%.dll Cntk.Core.JavaBinding-%version%.dll) do (
  copy "%output_dir%/%%x" ".\com\microsoft\CNTK\lib\windows\%%x" 
  echo %%x>> .\com\microsoft\CNTK\lib\windows\NATIVE_MANIFEST
)
